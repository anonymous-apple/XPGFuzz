#!/usr/bin/env python3
"""
Generate state transition coverage (edges) plots for all protocols with results.
...
def find_protocols_with_results(benchmark_dir):
    """Find all protocols that have states.csv files."""
    ...
    # Check each found protocol for states.csv
    for protocol in sorted(found_protocols):
        ...
        if states_file.exists():
            ...
            print(f"✓ Found state data for: {protocol}")
        else:
            print(f"✗ No states.csv found for: {protocol} (directory exists)")
...
def load_state_data(benchmark_dir, protocol):
    """Load state transition coverage (edges) data for a protocol."""
    benchmark_path = Path(benchmark_dir)
    ...
    # Filter for edges (state transition coverage)
    df_edges = df[df['state_type'] == 'edges'].copy()
...
def calculate_state_transition_coverage_over_time(df, time_limit=1440, step=10):
    """Calculate average state transition coverage (edges) over time for each fuzzer."""
    coverage_data = {}
    ...
    for fuzzer in FUZZERS:
        ...
        for run in runs:
            ...
            if run_data.empty:
                continue
            # Get initial state transition value (first data point)
            values_at_time_0.append(run_data.iloc[0]['state'])
        ...
        for time_min in range(step, time_limit + 1, step):
            ...
            for run in runs:
                ...
                # Get state transition coverage at or before target time
                time_filtered = run_data[run_data['time'] <= target_time]
                if not time_filtered.empty:
                    values_at_time.append(time_filtered['state'].iloc[-1])
...
def plot_state_transition_coverage_all(benchmark_dir, output_file=None, 
                            time_limit=1440, step=30):
    """Generate state transition coverage (edges) plots for all protocols."""
    ...
    if not protocols:
        print("\n" + "="*80)
        print("ERROR: No protocols with state data found!")
        print("="*80)
        print("\nTo generate states.csv files, you need to:")
    ...
    if output_file is None:
        ...
        output_file = f'state_transition_coverage_all_protocols_{date_str}.png'
    ...
    fig.suptitle('State Transition Coverage (Edges) Over Time - All Protocols', 
    ...
    for idx, protocol in enumerate(protocols):
        ...
        # Load data
        df = load_state_data(benchmark_dir, protocol)
        if df is None:
        ...
        # Calculate state transition coverage over time
        coverage_data = calculate_state_transition_coverage_over_time(df, time_limit, step)
        
        if not coverage_data:
            ax.text(0.5, 0.5, f'No state transition data for {protocol}', 
                   ha='center', va='center', transform=ax.transAxes)
            ax.set_title(PROTOCOLS.get(protocol, protocol), fontsize=12, fontweight='bold')
            continue
        ...
        ax.set_ylabel('#edges', fontsize=fontsize_xy)
        ax.set_title(PROTOCOLS.get(protocol, protocol), fontsize=12, fontweight='bold')
...
def main():
    ...
    plot_state_transition_coverage_all(benchmark_dir)
