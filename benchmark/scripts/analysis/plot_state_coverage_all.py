#!/usr/bin/env python3
"""
Generate state coverage (nodes) plots for all protocols with results.
...
def load_state_data(benchmark_dir, protocol):
    """Load state coverage (nodes) data for a protocol."""
    benchmark_path = Path(benchmark_dir)
    ...
    # Filter for nodes (state coverage)
    df_nodes = df[df['state_type'] == 'nodes'].copy()
...
def calculate_state_coverage_over_time(df, time_limit=1440, step=10):
    """Calculate average state coverage (nodes) over time for each fuzzer."""
    coverage_data = {}
    ...
    for fuzzer in FUZZERS:
        ...
        for run in runs:
            ...
            if run_data.empty:
                continue
            # Get initial state value (first data point)
        ...
        for time_min in range(step, time_limit + 1, step):
            ...
            for run in runs:
                ...
                # Get state coverage at or before target time
...
def plot_state_coverage_all(benchmark_dir, output_file=None, 
                            time_limit=1440, step=30):
    """Generate state coverage (nodes) plots for all protocols."""
    ...
    if output_file is None:
        ...
        output_file = f'state_coverage_all_protocols_{date_str}.png'
    ...
    fig.suptitle('State Coverage (Nodes) Over Time - All Protocols', 
    ...
    for idx, protocol in enumerate(protocols):
        ...
        # Calculate state coverage over time
        coverage_data = calculate_state_coverage_over_time(df, time_limit, step)
        
        if not coverage_data:
            ax.text(0.5, 0.5, f'No state data for {protocol}', 
                   ha='center', va='center', transform=ax.transAxes)
            ax.set_title(PROTOCOLS.get(protocol, protocol), fontsize=12, fontweight='bold')
            continue
        ...
        ax.set_ylabel('#nodes', fontsize=fontsize_xy)
        ax.set_title(PROTOCOLS.get(protocol, protocol), fontsize=12, fontweight='bold')
...
def main():
    ...
    plot_state_coverage_all(benchmark_dir)
