...
if $(strstr $FUZZER "afl") || $(strstr $FUZZER "llm") || $(strstr $FUZZER "xpgfuzz") ; then
  ...
  if $(strstr $FUZZER "xpgfuzz") && ! $(strstr $FUZZER "aflnet+s1"); then
    timeout -k 2s --preserve-status $TIMEOUT /home/ubuntu/${FUZZER}/afl-fuzz -b -d -i ${INPUTS} -x ${WORKDIR}/smtp.dict -o $OUTDIR -N tcp://127.0.0.1/25 $OPTIONS -c ${WORKDIR}/clean exim -bd -d -oX 25 -oP /var/lock/exim.pid
  else
    timeout -k 2s --preserve-status $TIMEOUT /home/ubuntu/${FUZZER}/afl-fuzz -d -i ${INPUTS}  -o $OUTDIR -N tcp://127.0.0.1/25 $OPTIONS -c ${WORKDIR}/clean exim -bd -d -oX 25 -oP /var/lock/exim.pid
  fi
  ...
  if [ $STATUS -eq $ORIGINAL_STATUS ]; then
    ...
    if [ -d "$CRASH_DIR" ] && [ "$(ls -A "$CRASH_DIR" 2>/dev/null)" ]; then
      ...
      PATH="${WORKDIR}:/usr/exim/bin:$PATH" python3 ${WORKDIR}/first_seen_crash.py \
        --out-dir ${OUTDIR} \
        --subdir ${CRASH_SUBDIR} \
        --proto SMTP \
        --port 25 \
        --host 127.0.0.1 \
        --server-cmd "bash -lc '${WORKDIR}/clean; exim -bd -d -oX 25 -oP /var/lock/exim.pid'" \
