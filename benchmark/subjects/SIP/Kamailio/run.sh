...
if $(strstr $FUZZER "afl") || $(strstr $FUZZER "llm") || $(strstr $FUZZER "xpgfuzz"); then
  ...
  if $(strstr $FUZZER "aflnet+s1"); then
    ...
    INPUTS=${INPUTS:-${WORKDIR}"/in-sip-x-old"}
  else
    INPUTS=${INPUTS:-${WORKDIR}"/in-sip"}
  fi
  ...
  if $(strstr $FUZZER "xpgfuzz") && ! $(strstr $FUZZER "aflnet+s1"); then
    timeout -k 2s --preserve-status $TIMEOUT /home/ubuntu/${FUZZER}/afl-fuzz -b -d -i ${INPUTS} -o $OUTDIR -N udp://127.0.0.1/5060 $OPTIONS -c ${WORKDIR}/run_pjsip ./src/kamailio -f ${WORKDIR}/kamailio-basic.cfg -L $KAMAILIO_MODULES -Y $KAMAILIO_RUNTIME_DIR -n 1 -D -E
  else
  ...
  if [ $STATUS -eq $ORIGINAL_STATUS ]; then
    ...
    if [ -d "$CRASH_DIR" ] && [ "$(ls -A "$CRASH_DIR" 2>/dev/null)" ]; then
      ...
      python3 ${WORKDIR}/first_seen_crash.py \
        ...
        --port 5060 \
        --host 127.0.0.1 \
        --server-cmd "./src/kamailio -f ${WORKDIR}/kamailio-basic.cfg -L $KAMAILIO_MODULES -Y $KAMAILIO_RUNTIME_DIR -n 1 -D -E" \
