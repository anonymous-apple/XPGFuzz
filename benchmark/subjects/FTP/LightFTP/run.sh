...
if $(strstr $FUZZER "afl") || $(strstr $FUZZER "llm") || $(strstr $FUZZER "xpgfuzz"); then
  ...
  if $(strstr $FUZZER "aflnet+s1"); then
    INPUTS=${WORKDIR}/in-ftp-1-4-night
  ...
  if $(strstr $FUZZER "xpgfuzz") && ! $(strstr $FUZZER "aflnet+s1"); then
    timeout -k 2s --preserve-status $TIMEOUT /home/ubuntu/${FUZZER}/afl-fuzz -b -d -i ${INPUTS} -x ${WORKDIR}/ftp.dict -o $OUTDIR -N tcp://127.0.0.1/2200 $OPTIONS -c ${WORKDIR}/ftpclean ./fftp fftp.conf 2200
  else
    timeout -k 2s --preserve-status $TIMEOUT /home/ubuntu/${FUZZER}/afl-fuzz -d -i ${INPUTS}  -o $OUTDIR -N tcp://127.0.0.1/2200 $OPTIONS -c ${WORKDIR}/ftpclean ./fftp fftp.conf 2200
  fi
  ...
  if [ $STATUS -eq $ORIGINAL_STATUS ]; then
    ...
    if [ -d "$CRASH_DIR" ] && [ "$(ls -A "$CRASH_DIR" 2>/dev/null)" ]; then
      ...
      python3 ${WORKDIR}/first_seen_crash.py \
        ...
        --port 2200 \
        --host 127.0.0.1 \
        --server-cmd "./fftp fftp.conf 2200" \
