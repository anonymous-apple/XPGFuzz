...
if $(strstr $FUZZER "afl") || $(strstr $FUZZER "llm") || $(strstr $FUZZER "xpgfuzz"); then
  ...
  if $(strstr $FUZZER "xpgfuzz") && ! $(strstr $FUZZER "aflnet+s1"); then
    timeout -k 2s --preserve-status $TIMEOUT /home/ubuntu/${FUZZER}/afl-fuzz -b -d -i ${INPUTS} -o $OUTDIR -x ${WORKDIR}/ftp.dict -N tcp://127.0.0.1/21 $OPTIONS -c ${WORKDIR}/clean ./bftpd -D -c ${WORKDIR}/basic.conf
  else
    timeout -k 2s --preserve-status $TIMEOUT /home/ubuntu/${FUZZER}/afl-fuzz -d -i ${INPUTS} -o $OUTDIR  -N tcp://127.0.0.1/21 $OPTIONS -c ${WORKDIR}/clean ./bftpd -D -c ${WORKDIR}/basic.conf
  fi
  ...
  if [ $STATUS -eq $ORIGINAL_STATUS ]; then
    ...
    if [ -d "$CRASH_DIR" ] && [ "$(ls -A "$CRASH_DIR" 2>/dev/null)" ]; then
      ...
      python3 ${WORKDIR}/first_seen_crash.py \
        ...
        --port 21 \
        --host 127.0.0.1 \
        --server-cmd "./bftpd -D -c ${WORKDIR}/basic.conf" \
