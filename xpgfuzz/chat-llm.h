...
/*
 * Paper ↔ Code mapping (Chapter 4: constraint-driven mutation + operator scheduling)
 *
 * - Type constraints: `type_constraint_t` and helpers (`parse_constraint()`,
 *   `generate_value_by_constraint()`, `mutate_value_by_constraint()`) implement
 *   template/constraint-aware mutations over protocol fields.
 * - Operator scheduling: the MAB/UCB1 data structures below support adaptive
 *   selection among multiple mutation operators, with reward updates triggered
 *   by coverage feedback in `xpgfuzz/afl-fuzz.c`.
 */
