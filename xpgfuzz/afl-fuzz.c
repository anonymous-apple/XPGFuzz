...
/* Adaptive epsilon scheduling (window-based pulse boost) */
static u32 adaptive_window_execs;
static u32 adaptive_window_gain;
static u32 adaptive_boost_windows_left;
static u8 adaptive_last_cov;
static u8 adaptive_last_state;
static double adaptive_epsilon = EPSILON_BASE;
...
void setup_llm_grammars()
{
  ...
  // Print all stored constraints for debugging
  print_all_constraints_storage();
}

/*
 * Paper ↔ Code mapping (Chapter 4: template/constraint-driven mutation)
 *
 * `parse_buffer()` decomposes a candidate request buffer into "mutation ranges"
 * using protocol-specific header/field regex patterns learned from templates.
 *
 * - Output: a `range_list` where each range may carry a `type_constraint_t*`
 *   (see `xpgfuzz/chat-llm.h`) describing semantic constraints such as
 *   INTEGER/STRING/ENUM/IP/PATH/HEX.
 * - Used by the exploit branch of havoc (see `havoc_exploit` stage) to guide
 *   mutations toward structurally valid protocol requests instead of mutating
 *   arbitrary bytes.
 */
...
static double clamp_double(double val, double min, double max)
{
  if (val < min)
    return min;
  if (val > max)
...
static void adaptive_log_event(const char *tag, double from, double to, u32 window_gain, u8 stalled)
{
  if (!out_dir)
    return;

  u8 *fname = alloc_printf("%s/epsilon_schedule.log", out_dir);
  FILE *f = fopen(fname, "a");
  if (f)
  {
    fprintf(f, "%llu execs=%llu %s from=%.4f to=%.4f gain=%u stalled=%u\n",
            get_cur_time(), total_execs, tag, from, to, window_gain, stalled);
    fclose(f);
  }
  ck_free(fname);
}

static void adaptive_reset_exec_flags(void)
{
  adaptive_last_cov = 0;
  adaptive_last_state = 0;
}

static void adaptive_record_cov_gain(void)
{
  adaptive_last_cov = 1;
}

static void adaptive_record_state_gain(u8 is_interesting, u8 dry_run)
{
  if (!dry_run && is_interesting)
    adaptive_last_state = 1;
}

static void adaptive_set_epsilon(double value)
{
  adaptive_epsilon = clamp_double(value, EPSILON_MIN, EPSILON_MAX);
}

/*
 * Paper ↔ Code mapping (Chapter 4: mutation scheduling / explore–exploit)
 *
 * `adaptive_get_epsilon()` exposes the current \epsilon_t used in the main havoc
 * loop to decide between:
 * - **exploration**: random byte-range mutations (full randomness)
 * - **exploitation**: grammar/constraint-aware mutations via `parse_buffer()`
 *
 * See the havoc stage (`havoc_explore` / `havoc_exploit`) for how \epsilon_t is
 * applied.
 */
static double adaptive_get_epsilon(void)
{
  return adaptive_epsilon;
}

static void adaptive_update_on_exec(void)
{
  if (!EPSILON_WINDOW)
    return;

  adaptive_window_execs++;
  if (adaptive_last_cov)
    adaptive_window_gain += 1;
  if (adaptive_last_state)
    adaptive_window_gain += EPSILON_STATE_LAMBDA;

  if (adaptive_window_execs < EPSILON_WINDOW)
    return;

  u8 stalled = adaptive_window_gain <= EPSILON_STALL_THETA;
  if (stalled)
    adaptive_boost_windows_left = EPSILON_BOOST_WINDOWS;

  double next_epsilon = adaptive_boost_windows_left ? EPSILON_BOOST : EPSILON_BASE;
  if (adaptive_boost_windows_left)
    adaptive_boost_windows_left--;

  next_epsilon = clamp_double(next_epsilon, EPSILON_MIN, EPSILON_MAX);
  if (adaptive_epsilon != next_epsilon || stalled)
    adaptive_log_event("epsilon_update", adaptive_epsilon, next_epsilon, adaptive_window_gain, stalled);

  adaptive_set_epsilon(next_epsilon);
  adaptive_window_execs = 0;
  adaptive_window_gain = 0;
}
...
void update_state_aware_variables(struct queue_entry *q, u8 dry_run)
{
  ...
  u8 is_interesting = is_state_sequence_interesting(state_sequence, state_count);
  adaptive_record_state_gain(is_interesting, dry_run);
  ...
  if (fault == crash_mode)
  {
    ...
    if (!(hnb = has_new_bits(virgin_bits)))
    {
      if (crash_mode)
        total_crashes++;
      /*
       * Paper ↔ Code mapping (Chapter 4: mutation operator scheduling)
       *
       * Reward signal for the Multi-Armed Bandit (UCB1) implemented in
       * `xpgfuzz/chat-llm.c`: we treat "new coverage discovered" as reward=1,
       * otherwise reward=0, and update the last selected operator accordingly.
       */
...
havoc_stage:
  ...
  /*
   * Paper ↔ Code mapping (Chapter 4: \epsilon-greedy scheduling)
   *
   * Draw \epsilon ~ U(0,1) and compare to \epsilon_t:
   * - explore: mutate over the entire buffer (no structural guidance)
   * - exploit: call `parse_buffer()` to obtain constraint-tagged ranges and
   *           prioritize semantics-preserving mutations (see below).
   */
  double epsilon = UR(100) / 100.0;
  double epsilon_t = adaptive_get_epsilon();

  int is_exploration = epsilon >= epsilon_t;
  if (is_exploration)
  {
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++)
  {
    ...
    for (i = 0; i < use_stacking; i++)
    {
      ...
      /*
       * Paper ↔ Code mapping (Chapter 4: constraint-driven mutation operator)
       *
       * If a range is tagged with a semantic constraint (derived from templates),
       * we mutate it using `mutate_value_by_constraint()` instead of generic byte
       * mutations to maintain protocol validity while exploring boundary values.
       */
