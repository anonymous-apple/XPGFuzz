...
void setup_llm_grammars()
{
  ...
  for (int iter = 0; iter < TEMPLATE_CONSISTENCY_COUNT; iter++)
  {
    ...
    char *templates_answer = chat_with_llm(templates_prompt, "gpt-3.5-turbo", GRAMMAR_RETRIES, 0.5);
    ...
    char *remaining_templates = chat_with_llm(remaining_prompt, "gpt-3.5-turbo", GRAMMAR_RETRIES, 0.5);
