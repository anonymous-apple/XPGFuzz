...
do
    ...
    # Generate unique bug statistics across all replications
    python3 scripts/analysis/unique_bug_stats.py \
        --results-dir results-${SUBJECT} \
        --subject ${SUBJECT}
    
    # Extract detailed bug artifacts (message sequences, logs, timing)
    python3 scripts/analysis/unique_bug_details.py --results-dir results-${SUBJECT} --subject ${SUBJECT} || true
